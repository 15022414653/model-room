...
void UMORTEditorWidgetClass::Setup(EMORTAction action, TArray<AActor*> actors, TArray<UObject*> assets, bool renameProjectAssets, bool renameWorldActors, FString textToFind, FString targetText, FString RegexPattern, int32 CapturingGroupNumber, int32 startNumber, bool numeratePosition, FString numerateSeparator, int32 charsToRemove)
{
	m_action = action;
	m_actors = actors;
	m_assets = assets;
	...
	m_numeratePosition = numeratePosition;
	m_numerateSeparator = numerateSeparator;
	m_charsToRemove = charsToRemove;

	if (m_renameProjectAssets)
	{
...
void UMORTEditorWidgetClass::OnEachSelectedAssetDelegate(UObject* asset, int32 index)
{
	...
	{
		if (m_numeratePosition)
		{
			newName = m_targetText + m_numerateSeparator + FString::FromInt(m_startNumber + index);
		}
		else
		{
			newName = FString::FromInt(m_startNumber + index) + m_numerateSeparator + m_targetText;
		}
	} break;

	case EMORTAction::Numerate:
	{
		if (m_numeratePosition)
		{
			newName = asset->GetName() + m_numerateSeparator + FString::FromInt(m_startNumber + index);
		}
		else
		{
			newName = FString::FromInt(m_startNumber + index) + m_numerateSeparator + asset->GetName();
		}
	} break;
...
void UMORTEditorWidgetClass::OnEachSelectedActorDelegate(AActor* actor, int32 index)
{
	...
	case EMORTAction::Numerate:
	{
		if (m_numeratePosition)
		{
			newName = actor->GetActorLabel() + m_numerateSeparator + FString::FromInt(m_startNumber + index);
			newName = TEXT(" ") + newName;
		}
		else
		{
			newName = FString::FromInt(m_startNumber + index) + m_numerateSeparator + actor->GetActorLabel();
			newName = TEXT(" ") + newName;
		}
	} break;
